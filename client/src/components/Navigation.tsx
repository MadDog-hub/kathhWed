...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b border-border shadow-soft transition-all duration-300 ${
      isVisible ? 'translate-y-0' : '-translate-y-full'
    }`} style={{ backgroundColor: '#4a3e35' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <div className="flex-shrink-0">
            <h2 className="text-xl font-display font-semibold text-[#ffffff]">
              R<span className="text-[#ffffff]">&</span>K
            </h2>
          </div>
            ...
            <div className="flex items-center space-x-1">
              {navItems.map((item) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 hover:bg-[#ffffff]/10 hover:text-[#ffffff] ${
      ...
      {isOpen && (
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 backdrop-blur-md border-b border-border" style={{ backgroundColor: '#4a3e35' }}>
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className={`block w-full text-left px-3 py-2 text-base font-medium rounded-md transition-all duration-300 hover:bg-[#ffffff]/10 hover:text-[#ffffff] ${
                  activeSection === item.href.slice(1)
                    ? 'text-[#ffffff] bg-[#ffffff]/5'
                    : 'text-foreground'
                }`}
